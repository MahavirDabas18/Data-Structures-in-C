...
int main()
{
    ...
    preorder(root);
    printf("\n");
    printf("the number of degree 2 nodes in the binary tree: %d \n",deg2count(root));
}
...
int deg2count(struct tnode* root)
{
    struct tnode* p=root;
    int x,y;
    if(p==NULL) /*base case*/
    ...
    {
        x=deg2count(p->lchild); /*assume that the functions works for input (n-1)*/
        y=deg2count(p->rchild);
        /*assuming the function works for (n-1) we formulate the result for the input n*/
        if(p->lchild!=NULL && p->rchild!=NULL)
        {
            return (x+y+1); /*the parent node is counted as it has both children*/
        }
        else
        {
            return (x+y); /*the parent node is not counted*/
        }
    }
}
