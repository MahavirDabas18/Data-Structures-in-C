...
int height(struct tnode* root)
{
    ...
    if(t==NULL) /*base case*/
    {
        return 0; /*if no node is present then height of the tree will be zero*/
    }
    else /*if there is a node present*/
    {
        int x,y;
        x=height(t->lchild); /*this will give height of the left subtree*/
        y=height(t->rchild); /*this will give height of the left subtree*/
        if(x>y)
        {
            return (x+1); /*if the height of the left subtree is more than the right subtree-the height of the tree would be this*/
        }
        else
        {
            return (y+1); /*if the height of the right subtree is more than the left subtree-the height of the tree would be this*/
        }
    }
}
