...
int main()
{
    ...
    preorder(root);
    printf("\n");
    printf("the sum of all nodes in the binary tree: %d \n",sumtree(root));
}
...
{
    struct tnode* t=root;
    if(t==NULL) /*if there is no node then the sum is zero*/
    {
        return 0;
    }
    ...
    {
        int x,y;
        x=sumtree(t->lchild); /*gives sum of nodes in left subtree*/
        y=sumtree(t->rchild); /*gives sum of nodes in left subtree*/
        return (x+y+t->data); /*the total sum of left subtree,right subtree and the parent node is returned*/
    }
}
